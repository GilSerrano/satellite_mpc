function [target_h,traj_h] = renderTarget(V,F,p,ang,target_h,traj_h, view_vector)
                                 
    V = rotate(V,ang);
    V = translate(V,p');

    if isempty(target_h) && isempty(traj_h)
    figure(1)
    traj_h = plot3(p(1),p(2),p(3),'--','color', [0 0.4470 0.7410],'Linewidth',1.5); 
    hold on;
    target_h = patch('Vertices', V', 'Faces', F, 'FaceColor','#55FF55');
%     view(3);
    view(view_vector);
    
    else
        set(target_h,'Vertices',V','Faces',F);
